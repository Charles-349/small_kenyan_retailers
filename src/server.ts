...
app.use("/api/assignments", riderAssignmentRoutes);
app.use("/api/delivery-status", deliveryStatusRoutes);
app.use("/api/deliveries", qrVerificationRoutes);
app.use("/api/rider", riderDeliveryRoutes);
app.use("/api/deliveries", dispatcherDeliveryRoutes);
app.use("/api/rider", riderLocationRoutes);
app.use("/api/deliveries", proofOfDeliveryRoutes);
app.use("/api/retailer", retailerDeliveryRoutes);

const PORT = Number(process.env.PORT);

server.listen(PORT, () => {
  console.log(
    `Server and WebSockets running on port ${PORT}`
  );
});